...
export default function SignUp() {
  ...
  async function submit() {
    ...
    if (form.password !== form.confirmPassword) {
      Alert.alert(
        t("Error"),
        t("The password and confirm password do not match.")
      );
      return;
    }
      ...
      <SafeAreaView className="flex-1 bg-primary">
            ...
            <View className="h-full justify-center px-6">
              <View className="justify-center mt-8">
                ...
                <CustomForm
                  ...
                  value={form.username}
                  placeholder={t("Give your account a catchy name")}
                />
                <CustomForm
                  title={t("Email")}
        ...
        >
          <View className="w-full h-full px-8 justify-center">
            <View className="mb-3 -mt-4">
              <Text className="text-2xl font-bold">{t("Verification")}</Text>
              <Text>{t("We sent a verification code to")}</Text>
              <Text>{form.email}</Text>
            </View>

            <Text className="text-xl font-semibold">{t("Code")}</Text>
        ...
        >
          <View className="w-full h-full px-8 items-center justify-center">
            <Image className="w-[100] h-[100] mb-10" source={check} />
            <Text className="text-xl font-bold mb-2 text-center">
              {t("Verification Successful")}
            </Text>

            <Text className="text-center text-gray-500 mb-10">
              {t("Auto-redirect to homepage after 2 seconds")}
            </Text>
          </View>
        </CustomModal>
